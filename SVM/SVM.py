...
def TrainAndPredict(xtrain, ytrain,train_Data_inFID,xval=None,yval=None,val_Data_inFID=None,xtest=None,ytest=None,test_Data_inFID=None,TestDistrict1X=None,TestDistrict1Y=None,TestDistrict1_inFID=None,TestDistrict2X=None,TestDistrict2Y=None,TestDistrict2_inFID=None,outFileFolder='',outFileName_BasedParam=''):
    ...
    cc,max_iter,gamma=10,-1,20 #0.3,-1,0.1   #默认值 classifier=svm.SVC(C=1,kernel='rbf',gamma=1,decision_function_shape='ovr')
    clf1=svm.SVC(C=cc,kernel='rbf',probability=True, max_iter=max_iter, gamma=gamma,decision_function_shape='ovr')
    outFileName_BasedParam='_cc'+str(cc)+'_max_iter'+str(max_iter)+'_gamma'+str(gamma)
    out_acc_lst=[]
    #混淆矩阵的准备工作(# Preparation of the confusion matrix)
    attack_types = ['1','2', '3', '4', '5', '6', '7']
...
def main():
    ...
    fea_np= np.load(r'data\SVM-BuildingFeanpy_.npy')
